...
ゲームモードには、「**シングルモード**」と「**バーサスモード**」の二種類があり、前者はArduino単体で動作する**スタンドアロン構成**で、後者はArduinoに搭載されている**ESP32のWiFi機能**を使用して**Node.js**で構築した**WebSocketサーバー**と**リアルタイム双方向通信**を行います。  
通信を**バイナリ形式**で行うことでデータ量を**大幅に削減**し、さらにゲームの更新処理に**タイマー割り込み**を用いることで**高いリアルタイム性**を確保しています。  
開発には**PlatformIO**を使用し**実務レベルのビルド管理・静的コード解析**を行いました。
...
- ゲームのリアルタイム性を向上させるため、タイマー割り込みを用いてゲームを更新
