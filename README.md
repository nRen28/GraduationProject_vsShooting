...
- 開発には**PlatformIO**を使用し**実務レベルのビルド管理・静的コード解析**を実施
- ゲームモードは、「**シングルモード**」と「**バーサスモード**」の二種類
- **シングルモード**はArduino単体で動作する**スタンドアロン構成**
- **バーサスモード**ではArduinoに搭載されている**ESP32のWiFi機能**を使用して**Node.js**で構築した**WebSocketサーバー**と**リアルタイム双方向通信**を行う
- Node.jsの**ノンブロッキングI/Oモデル**により、サーバーの**高い応答性**を実現
- 通信を**バイナリ形式**で行うことで**データ量を大幅に削減**し、さらにゲームの更新処理に**タイマー割り込み**を用いることで**高いリアルタイム性**を確保  
- **フラッシュメモリの仮想的なEEPROM領域**を活用し、**ハイスコアの永続保存**を実現
...
- リアルタイム性向上のためにWebSocketによる常時接続型通信を採用
